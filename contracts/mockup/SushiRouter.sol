...
library SushiLibrary {
            ...
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        hex"ecba335299a6693cb2ebc4782e74669b84290b6378ea3a3873c7231a8d7d1074" // Change to INIT_CODE_PAIR_HASH of Sushi Factory
                    )
                )
            )
        );
    ...
    function getReserves(
        ...
        (uint256 reserve0, uint256 reserve1, ) = ISushiPair(pairFor(factory, tokenA, tokenB)).getReserves();
...
contract SushiRouter is ISushiRouter02 {
    using SafeMath for uint256;
    ...
    function _addLiquidity(
        ...
        (uint256 reserveA, uint256 reserveB) = SushiLibrary.getReserves(factory, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint256 amountBOptimal = SushiLibrary.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, "SushiRouter: INSUFFICIENT_B_AMOUNT");
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint256 amountAOptimal = SushiLibrary.quote(amountBDesired, reserveB, reserveA);
    ...
    function removeLiquidity(
        ...
        (uint256 amount0, uint256 amount1) = ISushiPair(pair).burn(to);
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            ISushiPair(SushiLibrary.pairFor(factory, input, output)).swap(amount0Out, amount1Out, to, new bytes(0));
        }
    }
